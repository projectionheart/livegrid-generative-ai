...
function stop(message){session++;connected=false;engine=false;token='';pending={};$('key').value='';$('status').textContent=message||'Demo mode · nothing is connected';$('preview').hidden=true;$('demo').hidden=false;$('caption').textContent='DEMO VISUAL · NO AI RUNNING';if(imageURL)URL.revokeObjectURL(imageURL);imageURL=null;enabled()}
...
for(const k of ['x','y'])$(k).oninput=()=>{state[k]=Number($(k).value);$(k+'Value').textContent=state[k].toFixed(2);xy(state.x,state.y)};const pad=$('pad');function move(e){const r=pad.getBoundingClientRect();xy((e.clientX-r.left)/r.width,1-(e.clientY-r.top)/r.height)}pad.onpointerdown=e=>{pad.setPointerCapture(e.pointerId);move(e)};pad.onpointermove=e=>{if(pad.hasPointerCapture(e.pointerId))move(e)};pad.onpointerup=e=>{move(e);pad.releasePointerCapture(e.pointerId)};pad.onkeydown=e=>{const d={ArrowLeft:[-.02,0],ArrowRight:[.02,0],ArrowUp:[0,.02],ArrowDown:[0,-.02]}[e.key];if(d){e.preventDefault();xy(state.x+d[0],state.y+d[1])}};setInterval(refresh,1500);paint(state);
})();
